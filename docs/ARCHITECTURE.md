...
| File storage | **Jotform hosts all receipt images** (system of record). The local `uploads/` dir is only a transient buffer: an in-app submission keeps its image locally until forwarding to Jotform succeeds, then the local copy is deleted and only the Jotform URL is stored. | Server disk stays metadata-only (SQLite DB, a few MB), so a 1 GB Render disk never fills. Images are **never** served from public URLs; the app's `GET /api/receipts/:id/image` endpoint enforces RBAC and proxies the bytes from Jotform using the server-side API key. Note: receipt-image retention is therefore tied to Jotform — deleting a submission in Jotform deletes its image. |
